...
//  (253 k - 323k) (0.75 - 1.5 au) inclusive

function isHabitable(arr){
	let safePlanets = 
	planets.filter(planet => (planet.temperature >= 253 && planet.temperature <= 323 ) 
	&& 
	(planet.distance >= 0.75 && planet.distance <= 1.5));
	console.log(safePlanets)
}

isHabitable(planets)
// since filter returns what comes back as 'truthy' using AND statements to enusre planets are within paramaters seemd like the best soultion